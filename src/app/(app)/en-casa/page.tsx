...
import { PageHeader, Boton, PrioridadBadge, EstadoBadge, SeccionTitulo } from "@/components/ui";
import { ActionEditor } from "@/components/actions/action-editor";
import { WeeklyGroups } from "@/components/actions/weekly-groups";
import { Mas, Lapiz, Papelera, Tilde, Persona, Pata } from "@/components/icons";

/* Recordatorios afectivos cálidos para los seres queridos */
const AFECTOS = [
  { tipo: "persona", nombre: "Mamá", mensaje: "Hace 3 días que no hablás con ella" },
  { tipo: "persona", nombre: "Papá", mensaje: "Hablaste ayer" },
  { tipo: "persona", nombre: "Pablito", mensaje: "Cuida el depto — mandale un saludo" },
  { tipo: "pata", nombre: "Indi", mensaje: "Rosa la cuida — pedile una foto" },
  { tipo: "pata", nombre: "Benji", mensaje: "Todo bien por casa — ronronea fuerte" },
] as const;
...
export default function EnCasaPage() {
  ...
  const items = accionesVivas(data).filter((a) => a.group === "home");
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-4">
      ...
      <div className="grid gap-2.5 sm:grid-cols-2">
        {AFECTOS.map((p) => {
          const Icon = p.tipo === "pata" ? Pata : Persona;
          return (
            <div key={p.nombre} className="carta flex items-center gap-3 rounded-2xl p-3">
              <span className="grid h-10 w-10 flex-shrink-0 place-items-center rounded-full bg-[var(--casa-bg)] text-casa" aria-hidden="true">
                <Icon width={18} height={18} />
              </span>
              <div className="min-w-0 flex-1">
                <p className="font-serif text-marfil">{p.nombre}</p>
                <p className="text-xs text-gris-azul">{p.mensaje}</p>
              </div>
            </div>
          );
        })}
      ...
      {/* Trámites y pagos remotos — agrupados por semana */}
      ...
      <ActionEditor open={editor.open} onClose={() => setEditor({ open: false, editar: null })}
        fecha={new Date().toISOString().slice(0, 10)} editar={editor.editar} grupoInicial="home" />
    </div>
  );
}
